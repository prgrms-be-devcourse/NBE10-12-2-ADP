...
import com.back.standard.recommend.util.CosineSimilarity;
...
public class CosineSimilarityRecommendByReview implements RecommendByReview {
    ...
    static class Reviewer {
        ...
        List<VectorElement> getNormalizedRatings() {
            return ratings.entrySet().stream()
                    .map(entry ->
                            new VectorElement(entry.getKey(),
    ...
    private Map<Long, Reviewer> users;

    public void setData(Stream<RecommendByReview.RecommendReview> reviews) {
    ...
    public Stream<Long> getRecommend(long targetUserId, int referenceCnt, int maxRecommend) {

        Reviewer targetUser = users.getOrDefault(targetUserId, null);

        if (targetUser == null) return Stream.of();
        if (targetUser.ratings.isEmpty()) return Stream.of();

        CosineSimilarity cosineSimilarity = new CosineSimilarity();

        cosineSimilarity.setVectorA(targetUser.getNormalizedRatings());
        ...
        users.entrySet().stream()
                ...
                .forEach(set -> {
                    cosineSimilarity.setVectorB(set.getValue().getNormalizedRatings());
                    similars.add(new Similar(set.getKey(), cosineSimilarity.getCosineSimilarity()));
                });
